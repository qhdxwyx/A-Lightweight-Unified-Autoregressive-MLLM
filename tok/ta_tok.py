...
from . import models
from .utils import ScalingLayer, load_torch_checkpoint
...
class TextAlignedTokenizer(nn.Module):
    ...
    def encode(self, x, **kwargs):
        ...
        if hidden_states is not None and len(hidden_states) > 0:
            vq_feats = hidden_states[self.select_layer_id]
        else:
            vq_feats = getattr(encoder_outputs, "last_hidden_state", None)
            if vq_feats is None:
                raise RuntimeError("TA-Tok encoder did not return hidden_states or last_hidden_state.")
