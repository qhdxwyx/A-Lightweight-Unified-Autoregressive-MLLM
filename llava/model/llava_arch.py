        ...
        if num_new_tokens > 0:
            ...
            if model_args.load_embeddings_from_vision and vision_tower is not None:                
                ...
                if (
                    ...
                    and output_embeddings.shape[1] == vision_embeddings.shape[1]
                ):
                    rank0_print("Load vision embeddings from vision tower into input embeddings and lm_head.")
                    vision_embeddings = vision_embeddings.to(device=input_embeddings.device, dtype=input_embeddings.dtype)
                    input_embeddings[self.config.image_start_token_id:self.config.image_end_token_id+1] = vision_embeddings
                    output_embeddings[self.config.image_start_token_id:self.config.image_end_token_id+1] = vision_embeddings
                else:
                    rank0_print(
                        "Skip loading vision embeddings because shapes do not match: "
                        f"num_image_tokens={model_args.num_image_tokens}, "
                        f"vision_embeddings={tuple(vision_embeddings.shape)}, "
                        f"input_embeddings={tuple(input_embeddings.shape)}, "
                        f"output_embeddings={tuple(output_embeddings.shape)}"
                    )
